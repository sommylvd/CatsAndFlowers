...
namespace WpfApp1
{
    public class Cat
    {
        ...
        public void Work(Player player)
        {
            ...
            if (WorkType == WorkPlaceName.FlowerMeadow)
            {
                for (int i = 0; i < SkillLevel.WorkSpeed; i++)
                {
                    player.FlowersInStock.Add(Storage.PickFlower(SkillLevel.QualificationName));
                }

            }
            if (WorkType == WorkPlaceName.CraftMeadow)
            {
